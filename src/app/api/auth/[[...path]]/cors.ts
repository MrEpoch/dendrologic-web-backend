// Original source: https://github.com/vercel/examples/blob/main/edge-functions/cors/lib/cors.ts
// Modified to pass linting
/**
...
function getAllowedHeaders(req: Request, confAllowedHeaders?: string | string[]) {
  const headers = new Headers()
  
  let allowedHeaders: string;
  if (!confAllowedHeaders) {
    allowedHeaders = req.headers.get('Access-Control-Request-Headers')!
    headers.append('Vary', 'Access-Control-Request-Headers')
  } else if (Array.isArray(confAllowedHeaders)) {
    // If the allowed headers is an array, turn it into a string
    allowedHeaders = confAllowedHeaders.join(',')
  } else {
    allowedHeaders = confAllowedHeaders;
  }
