...
transform(
  ...
  function deleteSelectedFeature() {
    mapStateRef.current.getLayers().getArray().forEach((layer: any) => {
      const source = layer.getSource();
      if (source instanceof VectorSource) {
        const featureExists = source.getFeatures();
        source.removeFeature(featureExists.find((f: any) => {
          return f.getProperties().geometry.ol_uid === selectedFeature?.getProperties().geometry.ol_uid
        }));
  ...
  async function saveRequest() {
    const geoJSONdata = mapStateRef.current.getLayers().getArray().map((layer: any) => {
      const source = layer.getSource();
      if (source instanceof VectorSource) {
  ...
  return (
    <div className="py-8 flex flex-col gap-4">
      ...
      <div className="flex gap-2 justify-between items-center">
        ...
        <Button className={`h-4 w-4 p-4 rounded border ${circling ? 'bg-main-background-300' : 'bg-main-background-100'}`} variant="outline" onClick={() => setCircling(prev => !prev)}>
          <Pen />
        </Button>
      </div>
      <div className="flex gap-2 justify-between items-center">
        <Label htmlFor="circling">Vytvořit žádost</Label>
        <Button className={`h-4 w-4 p-4 rounded border`} variant="outline" onClick={saveRequest}>
          <Check />
        </Button>
      </div>
  ...
  <DialogContent>
    ...
    <Button onClick={deleteSelectedFeature} variant="destructive">Smazat kruh</Button>
  </DialogContent>
</Dialog>
