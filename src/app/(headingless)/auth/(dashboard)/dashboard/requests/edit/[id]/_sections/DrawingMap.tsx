...
export default function DrawingMap({ requestInfo }) {
  ...
  useEffect(() => {
    ...
    if (mapStateRef.current === null) {
      ...
      const transformedData = JSON.parse(requestInfo.geodata[0]);
  ...
  async function updateRequest() {
    try {
      ...
      if (data.success) {
        await localStorage.deleteItem("geoJSONdata-" + geoRequestName);
  ...
  function removeSavedSnapshotFromLocalStorage() {
    try {
      ...
      localStorage.removeItem("geoJSONdata-" + requestInfo.id);
    } catch (error) {
      toast({
  ...
  return (
    <div className="py-8 flex flex-col gap-4">
      ...
      <div className="flex gap-2 justify-between items-center">
        <Label htmlFor="vytvor-zadosti">Aktualizovat žádost</Label>
        <Button
          className={`h-4 w-4 p-4 rounded border`}
          variant="outline"
